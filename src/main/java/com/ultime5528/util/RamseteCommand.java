...
public class RamseteCommand extends CommandBase {
  ...
  public void initialize() {
    ...
    // m_transform = m_pose.get().minus(initialState.poseMeters);
    // m_transform = initialState.poseMeters.minus(m_pose.get());
    var initialPose = m_pose.get();

    // m_transform = new Transform2d(initialPose.getTranslation().minus(initialState.poseMeters.getTranslation()), new Rotation2d(initialPose.getRotation().getRadians() - initialState.poseMeters.getRotation().getRadians()));

    m_transform = initialPose.minus(initialState.poseMeters);
    m_currentTrajectory =  m_trajectory.transformBy(m_transform);
  ...
  public void execute() {
    ...
    var targetWheelSpeeds = m_kinematics.toWheelSpeeds(
        m_follower.calculate(m_pose.get(), state));
